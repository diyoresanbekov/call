...
export default function AppPreview() {
  ...
  return (
    <div className="w-full overflow-hidden rounded-xl2 border border-border bg-surface text-ink shadow-window-frame">
      <div className="flex items-center justify-between border-b border-border bg-surface-3/70 px-4 py-3">
      ...
      <div className="text-sm lg:grid lg:grid-cols-[220px_1fr]">
        <div className="border-b border-border bg-canvas p-3 lg:border-b-0 lg:border-r">
            ...
            {sipAccounts.map((acc) => (
              <li key={acc.name} className="shrink-0 lg:shrink">
                <button
                  ...
                  className="group flex w-full items-center gap-2.5 rounded-lg border border-border bg-surface px-3 py-2 text-left transition duration-200 hover:border-signal/40 hover:bg-signal-faint active:scale-[0.98]"
                >
          ...
          <div className="mt-4 hidden border-t border-border pt-3 lg:block">
            <button
              type="button"
              className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-xs text-ink-muted transition duration-200 hover:bg-surface hover:text-ink"
            >
        ...
        <div className="bg-canvas p-4 sm:p-5">
          <div className="mb-4">
            ...
            <div className="flex items-center justify-between rounded-xl border border-border bg-surface px-4 py-3 font-mono text-lg tracking-wider text-ink shadow-[0_1px_2px_rgba(24,24,27,0.04)] sm:text-xl">
          ...
          <div className="mb-5 grid grid-cols-3 gap-2">
            {["1", "2", "3", "4", "5", "6", "7", "8", "9", "*", "0", "#"].map((key) => (
              <button
                ...
                className="min-h-11 rounded-lg border border-border bg-surface py-3 font-mono text-base font-medium text-ink transition duration-200 hover:border-signal/40 hover:bg-signal-faint active:scale-95"
              >
                {key}
              </button>
            ))}
          ...
          <div className="mb-6 flex items-center gap-2">
            <button
              type="button"
              className="flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-live py-3 text-sm font-semibold text-white transition duration-200 hover:bg-live-dim active:scale-95"
            >
            ...
            <button
              type="button"
              aria-label="Mikrofon"
              className="min-h-11 rounded-xl border border-border bg-surface p-3 text-ink-muted transition duration-200 hover:bg-surface-3 hover:text-ink"
            >
          ...
          <div>
            ...
            <ul className="space-y-1">
              {recentCalls.map((call, i) => (
                <li
                  key={i}
                  className="flex items-center justify-between rounded-lg border border-transparent px-3 py-2 transition duration-200 hover:border-border hover:bg-surface"
                >
